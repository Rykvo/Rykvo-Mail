...
class H(BaseHTTPRequestHandler):
 ...
 def sendh(self,x,n=200,c=None):
  b=x.encode();self.send_response(n);self.send_header("Content-Type","text/html; charset=utf-8");self.send_header("Content-Length",len(b));self.send_header("X-Frame-Options","DENY");self.send_header("Cache-Control","no-store")
  if c:self.send_header("Set-Cookie",c)
  self.end_headers();self.wfile.write(b)
 def sendj(self,x,n=200):
  b=json.dumps(x,ensure_ascii=False).encode();self.send_response(n);self.send_header("Content-Type","application/json; charset=utf-8");self.send_header("Content-Length",len(b));self.send_header("Cache-Control","no-store");self.end_headers();self.wfile.write(b)
 def sendsvg(self,x):
 ...
 def do_POST(self):
  ...
  if p==ADMIN_PATH:
   ...
   if hmac.compare_digest(str(f.get('user','')),au)and hmac.compare_digest(str(f.get('password','')),ap):
    t=token();self.send_response(303);self.send_header('Location',ADMIN_PATH);self.send_header('Set-Cookie',f'mp={t}; Path=/; HttpOnly; SameSite=Lax');self.end_headers();return
